...
*/ 
/** 
* Class iHRIS_Module_ManageAccessInstitutionType
* 
* @access public
*/


class iHRIS_Module_ManageAccessInstitutionType extends I2CE_ModuleAccess {

    /**
     * @var array A cached list of institution_type_access for a username
     */
    protected static $user_access_institution_type;
    ...
    public static function getMethods() {
        return array(
                'iHRIS_PageViewUser->action_access_institution_type' => 'action_access_institution_type',
                );
    }
    ...
    public function get_report_limit_options() {
        if ( !self::$report_limit_options 
            ...
            $facAccess = $factory->createContainer( "access_institution_type" );
            ...
            self::$report_limit_options = array(
                    'module' => "ManageAccessInstitutionType",
                    'fields' => $fields,
                    );
        }
    ...
    /**
     * Handle the display for the access_institution_type form on the view user page.
     * @return boolean
     */
    public function action_access_institution_type( $page ) {
        if ( !$page instanceof iHRIS_PageViewUser ) {
            return;
        }
        $template = $page->getTemplate();
        $template->appendFileById( "user_view_link_access_institution_type.html", "li", "user_edit_links" );

        $view_user = $page->getViewUser();
        $view_user->populateChildren("access_institution_type");
        if ( array_key_exists( 'access_institution_type', $view_user->children ) 
                && is_array( $view_user->children['access_institution_type'] ) ) {
            $node = $template->appendFileById( "user_view_access_institution_type_top.html", "div", "user_child_forms" );
            foreach( $view_user->children['access_institution_type'] as $child ) {
                $node = $template->appendFileById( "user_view_access_institution_type.html", "div", "access_institution_type" );
                if ( !$node instanceof DOMNode ) {
                    I2CE::raiseError( "Could not find template user_view_access_institution_type.html" );
                    return false;
                }
    ...
    public static function getAccessLocation( $user ) {
        ...
        if ( !is_array( self::$user_access_institution_type ) ) {
            self::$user_access_institution_type = array();
        }
        if ( !isset( self::$user_access_institution_type[$username] ) ) {
            self::$user_access_institution_type[$username] = array();
            $user = I2CE_FormFactory::instance()->createContainer( "user|$username" );
            $user->populateChildren( "access_institution_type" );
            if ( array_key_exists( 'access_institution_type', $user->children ) 
                    && is_array( $user->children['access_institution_type'] ) ) {
                foreach( $user->children['access_institution_type'] as $child ) {
                    $ff = $child->getField("location");
                    self::$user_access_institution_type[$username][$ff->getMappedForm()][] =
                        $ff->getDBValue();
                }
            }
        }
        return self::$user_access_institution_type[$username];
    }
     ...
     * @param string $link
     */
    public function userAccessInstitutionType($node,$template) {
        // This should only work for the institution_type_manager role so ignore any others.
        if ( $template->getUser()->getRole() != "institution_type_manager" ) {
            return false;
        }
        ...
        $fieldWalks['position']='institution_type';
     ...
     * @param string $link
     */
    public function userAccessInstitutionTypeStaff($node,$template) {
        // This should only work for the institution_type_manager role so ignore any others.
        if ( $template->getUser()->getRole() != "institution_type_staff" ) {
            return false;
        }
        if ( !$template instanceof I2CE_Template) {
            return false;
     ...
     * @param string $link
     */
    public function userAccessInstitutionTypeDataEntrant($node,$template) {
        // This should only work for the institution_type_manager role so ignore any others.
        if ( $template->getUser()->getRole() != "institution_type_data_entrant" ) {
            return false;
        }
        if ( !$template instanceof I2CE_Template) {
            return false;
     ...
     * @returns boolean
     */
    public function userAccessInstitutionTypeList($node,$template) {
        // This should only work for the institution_type_manager role so ignore any others.
        if ( $template->getUser()->getRole() != "institution_type_manager" ) {
            return false;
        }
        $fieldWalks = array(
            'position'=> 'institution_type'
            ,'institution_type'=> 'location'
            ,'county' => 'district'
            ,'district'=> 'region'
            ,'region' => 'country'
