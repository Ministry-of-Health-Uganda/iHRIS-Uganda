 ...
 * @access public
 */
class iHRIS_PageUgandaDistrictsAuditOpenSummary extends I2CE_Page {
    ...
    protected function action() {
        parent::action();
        $this->template->setAttribute( "class", "active", "menuAuditPage", "a[@href='audit_summary_districts']" );
    ...
    protected function action_district() {
        ...
        try {
            $stmt = $db->prepare( $qry);
            $stmt->execute();
            $this->template->addFile( "audit_summary_list.html" );
            ...
            foreach( $districts as $district ) {
                if ( $found[ $district['value'] ] ) {
                    $node = $this->template->appendFileById( "audit_summary_list_line.html",
                            "li", "district_list" );
