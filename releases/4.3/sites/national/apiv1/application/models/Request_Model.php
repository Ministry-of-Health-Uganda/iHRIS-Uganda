...
Class Request_Model extends CI_Model
{
    ...
    public function practitioner_data($district){
        ...
        $result = $this->db->query("SELECT s.* ,  TO_BASE64(a.`Photo+image`) as `imagedata` FROM `zebra_staff_list` s  join `zebra_staff_album` a ON a.`Photo+id`=s.`Photo+id` LIMIT 2")->result();
