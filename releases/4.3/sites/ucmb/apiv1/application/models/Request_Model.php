...
Class Request_Model extends CI_Model
{
    ...
    public function getihrisdata()
    {
        ...
        CONCAT('UCMB-',`facility+facility_type`) as facility_type_id,
        CONCAT('UCMB-',trim(`facility+id`)) as facility_id,
       ...
       `job+title` as job,
        ...
        from  `zebra_staff_list` WHERE `national_id_card_no+id_num` IS NOT NULL ");
    return $query->result();
    ...
    public function practitioner_data($offset=FALSE, $page_limit=FALSE){
        ...
        $result = $this->db->query("SELECT * FROM `zebra_ihris_data_api` where `national_id+id_num`!='' LIMIT $offset, $page_limit")->result_array();
