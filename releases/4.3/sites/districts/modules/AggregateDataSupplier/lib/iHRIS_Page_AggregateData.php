 ...
 dhis    */
	...
	*
	...
	if (!I2CE_CustomReport::reportExists($this->args['report'])) {
		...
		$qryDataValues = "SELECT `dhis_orgunit+code` AS dhis_orgunit, `job+code` AS dataelement, `primary_form+amount` AS norm, `+filled_positions` AS filled FROM `zebra_staff_norm` WHERE (`primary_form+amount` !=0 OR `+filled_positions`!=0) AND `district+name`='".$this->district."'  ORDER BY `district+name`,`facility+name`";
