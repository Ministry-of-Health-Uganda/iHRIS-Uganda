...
class iHRIS_PageUgandaDistrictsAuditReport extends I2CE_Page {
    ...
    protected function action_display( $district=null ) {
	    ...
	    $db = I2CE::PDO();
        $qry = "SELECT `job+title` AS title, `salary_grade+name` AS salary_grade,`facility+id` AS fID, `facility+name` AS fName, `primary_form+amount` AS amount , `+filled_positions` AS filled_positions , `+variance` AS variance, `+percentage_filled` AS percentage_filled FROM `zebra_staff_norm` WHERE `district+id` = '" . I2CE_PDO::escape_string( $district ) . "' ORDER BY `facility+name`, `salary_grade+id`, `job+title`";
            ...
            while( $data = $stmt->fetch() ) {
                if ( $data->fID != $last_facility ) {
		            ...
		            if ($data->amount == 0 && $data->filled_positions == 0  ){
                        ...
                        $this->template->setDisplayDataImmediate( 'facility_name', $data->fName, $facilityNode );
