...
class iHRIS_Page_UploadAttendanceCli extends I2CE_PageFormCSV {
    ...
    protected function saveRow( $key ) {
        ...
        if (isset($this->current[$key]['row']['timeIn']) || isset($this->current[$key]['row']['timeOut']) ){
        $time_in = $this->arrange_time($this->current[$key]['row']['timeIn']);
        $time_out = $this->arrange_time($this->current[$key]['row']['timeOut']);
	...
	echo "Clockin Exitsed " .$clockin_id . "\n" ;
		$clockin = $this->factory->createContainer( "clockin|$clockin_id" );
		    ...
		    echo "New time in less than Old time in \n";
                    $clockin->getField('time_in')->setFromDB($time_in);
                    }
                    if ( $old_time_out <  $time_out ) {
		    echo "Old time out less than new time out \n";
                    $clockin->getField('time_out')->setFromDB($time_out);
                    }
                $clockin->save( $this->user );
                $clockin->cleanup();
